//BackendCalls request requirements here
...
const APP_ID = "ENTER EDAMAM APP ID";
const API_KEY = "ENTER EDAMAM KEY";
...
function convertToNTR(name) {
  return NTR[Nutrition.indexOf(name)];
}

function searchBar_search_get(req, res, next) {
  let param = req.query.term;
  let filter = req.query.filter;

  let url = `https://api.edamam.com/search?q=${param}&app_id=${APP_ID}&app_key=${API_KEY}`;
  if (filter !== undefined) {
    {
      filter.map((e, i) => {
        let obj = JSON.parse(e);
        url = url.concat(
          `&nutrients[${convertToNTR(obj.nut)}]=${obj.min}-${obj.max}`
        );
      });
    }
  }

  //API CALL DISABLED TEMP
  axios
    .get(url)
  ...
  // let returnRecipe = [
  //   {
  //     recipe: TeriyakiChicken
  //   }
  // ];
  // res.send(returnRecipe);
}

//
// let returnFinalQuery = [];
// let outerElements = returnDatabaseQuery.length;
// console.log(outerElements);
// if(outerElements === 0){
//   let returnDummyQuery = [
//     [
//       ["NO STORED MEAL PLANS"],[],[],[],[],[],[],[]
//     ]
//   ];
//   res.send(returnDummyQuery);
// }
//
// returnDatabaseQuery.map((inputQuery, index) => {
//   let elements = -1;
//   inputQuery.map((subArray) =>{
//     elements = subArray.length + elements;
//   });
//
//   let returnQuery = [
//     [], //MEAL PLAN NAME
...
//   ];
//
//   inputQuery.map((subArray, index) => {
//     if(index > 0){
//       subArray.map((elem) => {
//         let uri = elem;
//         let start = uri.indexOf("_");
//         let end = uri.length;
//         let list = uri.substring(start, end);
//         let search_uri =
//           "http:%2F%2Fwww.edamam.com%2Fontologies%2Fedamam.owl%23recipe" + list;
//         const url = `https://api.edamam.com/search?r=${search_uri}&app_id=${APP_ID}&app_key=${API_KEY}`;
//         axios
//           .get(url)
//           .then(function (response) {
//             returnQuery[index].push(response.data);
//             elements--;
//             if (elements === 0) {
//               returnFinalQuery.push(returnQuery);
//               outerElements--;
//               console.log(outerElements);
//               if(outerElements === 0){
//                  console.log(returnFinalQuery);
//                  res.send(returnFinalQuery);
//                }
//             }
//           })
//           .catch(function (e) {
...
//       });
//     } else {
//       returnQuery[0].push(inputQuery[0]);
//       if(outerElements === 0 && elements === 0){
//         res.send(returnFinalQuery);
//       }
//     }
//   });
...
function searchURI(req, res, next) {
  console.log("SEARCHING BASED ON URI");
  ...
  /*
  let returnRecipe = [TeriyakiChicken];
  res.send(returnRecipe);
  */
}

const TeriyakiChicken = {
  uri:
    "http://www.edamam.com/ontologies/edamam.owl#recipe_7bf4a371c6884d809682a72808da7dc2",
  ...
  url:
    "http://www.davidlebovitz.com/2012/12/chicken-teriyaki-recipe-japanese-farm-food/",
  shareAs:
    "http://www.edamam.com/recipe/teriyaki-chicken-7bf4a371c6884d809682a72808da7dc2/-",
  yield: 6,
  dietLabels: ["Low-Carb"],
  healthLabels: [
    "Sugar-Conscious",
...
module.exports = {
  searchBar: searchBar_search_get,
  searchURI: searchURI
};
